<?php
$email = "wkerswell@gmail.com";
$fromEmail = "wkerswell@gmail.com";
mail ( $email , "IPN reached" , "Hello");
// Check to see there are posted variables coming into the script
if ($_SERVER['REQUEST_METHOD'] != "POST") die (mail ( $email , "no post vars" , "Hello"););
...
mail($email, "NORMAL IPN RESULT YAY MONEY!", $req);
...
if (strpos($curl_result, "VERIFIED") !== false) {
	...
	mail($email, "IPN interaction not verified", $req );
	exit();
}
...
if ($receiver_email != $email) {
    ...
    mail($email, "Receiver Email is incorrect", $message );
    exit(); // exit script
}
// Check number 2 ------------------------------------------------------------------------------------------------------------
...
if ($numRows > 0) {
    ...
    mail($email, "Duplicate txn_id in the IPN system", $message );
    exit(); // exit script
} 
// Check number 4 ------------------------------------------------------------------------------------------------------------
...
if ($fullAmount != $grossAmount) {
        ...
        mail($email, "Price Jack or Bad Programming", $message );
        exit(); // exit script
} 
...
$custom = $_POST['custom'];
$first_name = $_POST['custom'];
$last_name= $_POST['custom'];
$payment_date= $_POST['custom'];
$mc_gross= $_POST['custom'];
$payment_currency = $_POST['custom'];
$txn_id= $_POST['custom'];
$receiver_email= $_POST['custom'];
$payment_type= $_POST['custom'];
$payment_status= $_POST['custom'];
$txn_type= $_POST['custom'];
$payer_status= $_POST['custom'];
$address_street= $_POST['custom'];
$address_city= $_POST['custom'];
$address_state= $_POST['custom'];
$address_zip= $_POST['custom'];
$address_country= $_POST['custom'];
$address_status= $_POST['custom'];
$notify_version= $_POST['custom'];
$verify_sign= $_POST['custom'];
$payer_id= $_POST['custom'];
$mc_currency= $_POST['custom'];
$mc_fe= $_POST['custom'];
